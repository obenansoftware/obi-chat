...
async def update_agent(
    ...
    try:
        ...
        if is_suna_agent:
            logger.warning(f"Update attempt on Obi Chat default agent {agent_id} by user {user_id}")
            
            if (agent_data.name is not None and 
                agent_data.name != existing_data.get('name') and 
                restrictions.get('name_editable') == False):
                logger.error(f"User {user_id} attempted to modify restricted name of Obi Chat agent {agent_id}")
                raise HTTPException(
                    status_code=403, 
                    detail="Obi Chat's name cannot be modified. This restriction is managed centrally."
                )
            ...
            if (agent_data.description is not None and
                ...
                logger.error(f"User {user_id} attempted to modify restricted description of Obi Chat agent {agent_id}")
                raise HTTPException(
                    status_code=403, 
                    detail="Obi Chat's description cannot be modified."
                )
            
            if (agent_data.system_prompt is not None and 
                restrictions.get('system_prompt_editable') == False):
                logger.error(f"User {user_id} attempted to modify restricted system prompt of Obi Chat agent {agent_id}")
                raise HTTPException(
                    status_code=403, 
                    detail="Obi Chat's system prompt cannot be modified. This is managed centrally to ensure optimal performance."
                )
            
            if (agent_data.agentpress_tools is not None and 
                restrictions.get('tools_editable') == False):
                logger.error(f"User {user_id} attempted to modify restricted tools of Obi Chat agent {agent_id}")
                raise HTTPException(
                    status_code=403, 
                    detail="Obi Chat's default tools cannot be modified. These tools are optimized for Obi Chat's capabilities."
                )
            
            if ((agent_data.configured_mcps is not None or agent_data.custom_mcps is not None) and 
                restrictions.get('mcps_editable') == False):
                logger.error(f"User {user_id} attempted to modify restricted MCPs of Obi Chat agent {agent_id}")
                raise HTTPException(
                    status_code=403, 
                    detail="Obi Chat's integrations cannot be modified."
                )
            
            logger.info(f"Obi Chat agent update validation passed for agent {agent_id} by user {user_id}")
