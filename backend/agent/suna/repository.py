...
class SunaAgentRepository:
    ...
    async def find_all_suna_agents(self) -> List[SunaAgentRecord]:
        try:
            ...
            logger.error(f"Failed to find Obi Chat agents: {e}")
            raise
    ...
    async def create_suna_agent_simple(
        ...
        try:
            ...
            if result.data:
                ...
                logger.info(f"Created minimal Obi Chat agent {agent_id} for {account_id}")
            ...
            logger.error(f"Failed to create Obi Chat agent for {account_id}: {e}")
            raise
    ...
    async def _create_initial_version(
        ...
        try:
            ...
            await version_service.create_version(
                ...
                version_name="v1",
                change_description="Initial Obi Chat agent version"
            )
            
            logger.info(f"Created initial version for Obi Chat agent {agent_id}")
            
        except Exception as e:
            logger.error(f"Failed to create initial version for Obi Chat agent {agent_id}: {e}")