...
# Defer Daytona client initialization when credentials are missing to avoid startup crash
if daytona_config.api_key:
    logger.debug("Daytona API key configured successfully")
    daytona = AsyncDaytona(daytona_config)
else:
    logger.warning("No Daytona API key found in environment variables - sandbox features disabled")
    daytona = None
...
async def get_or_start_sandbox(sandbox_id: str) -> AsyncSandbox:
    ...
    try:
        if daytona is None:
            raise RuntimeError("Sandbox provider not configured: Daytona API key is missing")
