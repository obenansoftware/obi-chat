#!/usr/bin/env python3
"""
Obi Chat Default Agent Management Script

This script provides administrative functions for managing Obi Chat default agents across all users.

Usage:
    # 🚀 EASY COMMANDS (Most common)
    python manage_suna_agents.py sync                  # Push config changes to all users (recommended)
    python manage_suna_agents.py install-all          # Install Obi Chat for all users who don't have it
    python manage_suna_agents.py stats                # Show Obi Chat agent statistics
    
    # 🔧 ADVANCED COMMANDS
    python manage_suna_agents.py update-all           # Update all Obi Chat agents to latest version
    python manage_suna_agents.py install-user <id>    # Install Obi Chat for specific user
    python manage_suna_agents.py update-user <id>     # Update Obi Chat agent for specific user
...
class SunaAgentManager:
    ...
    async def sync_config(self):
        ...
        print("🔄 Syncing Obi Chat configuration from suna_config.py to all users...")
        print("📝 This will update system prompt, tools, and settings for all Obi Chat agents")
        ...
        if result['updated_count'] > 0:
            ...
            print("💡 All users now have the latest Obi Chat configuration from suna_config.py")
    
    async def install_all_users(self):
        """Install Obi Chat agent for all users who don't have it"""
        print("🚀 Installing Obi Chat default agent for all users who don't have it...")
        ...
        if result['installed_count'] > 0:
            print(f"\n✅ Successfully installed Obi Chat for {result['installed_count']} users")
            
    async def update_all_agents(self, target_version=None):
        """Update all Obi Chat agents to latest or specific version"""
        version_text = target_version or "latest"
        print(f"🔄 Updating all Obi Chat default agents to {version_text} version...")
        ...
        if result['updated_count'] > 0:
            print(f"\n✅ Successfully updated {result['updated_count']} Obi Chat agents")
    
    async def install_user(self, account_id):
        """Install Obi Chat agent for specific user"""
        print(f"🚀 Installing Obi Chat default agent for user {account_id}...")
        
        agent_id = await self.service.install_suna_agent_for_user(account_id)
        
        if agent_id:
            print(f"✅ Successfully installed Obi Chat agent {agent_id} for user {account_id}")
        else:
            print(f"❌ Failed to install Obi Chat agent for user {account_id}")
    
    async def update_user(self, account_id):
        """Update Obi Chat agent for specific user"""
        print(f"🔄 Updating Obi Chat default agent for user {account_id}...")
        ...
        if agent_id:
            print(f"✅ Successfully updated Obi Chat agent {agent_id} for user {account_id}")
        else:
            print(f"❌ Failed to update Obi Chat agent for user {account_id}")
    
    async def show_stats(self):
        """Show Obi Chat agent statistics"""
        print("📊 Obi Chat Default Agent Statistics")
        print("=" * 50)
...
async def main():
    parser = argparse.ArgumentParser(
        description="Manage Obi Chat default agents across all users",
    ...
    subparsers.add_parser('install-all', help='Install Obi Chat agent for all users who don\'t have it')
    subparsers.add_parser('stats', help='Show Obi Chat agent statistics')
    
    # 🔧 ADVANCED COMMANDS  
    subparsers.add_parser('update-all', help='Update all Obi Chat agents to latest version')
    
    # Install user command
    install_user_parser = subparsers.add_parser('install-user', help='Install Obi Chat agent for specific user')
    install_user_parser.add_argument('account_id', help='Account ID to install Obi Chat for')
    
    # Update user command
    update_user_parser = subparsers.add_parser('update-user', help='Update Obi Chat agent for specific user')
    update_user_parser.add_argument('account_id', help='Account ID to update Obi Chat for')
