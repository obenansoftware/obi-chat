...
class SunaDefaultAgentService:
    ...
    async def install_suna_agent_for_user(self, account_id: str, replace_existing: bool = False) -> Optional[str]:
        logger.info(f"🔄 Installing Obi Chat agent for user: {account_id}")
        ...
        try:
            if replace_existing:
                ...
                if existing:
                    ...
                    logger.info(f"Deleted existing Obi Chat agent for replacement")
            else:
                agents = await self._sync_service.repository.find_all_suna_agents()
                existing = next((a for a in agents if a.account_id == account_id), None)
                if existing:
                    logger.info(f"User {account_id} already has Obi Chat agent: {existing.agent_id}")
                    return existing.agent_id

            agent_id = await self._sync_service.repository.create_suna_agent_simple(account_id)
            
            logger.info(f"Successfully installed Obi Chat agent {agent_id} for user {account_id}")
            return agent_id
