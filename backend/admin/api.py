...
async def admin_install_suna_for_user(
    ...
    logger.info(f"Admin installing Obi Chat agent for user: {account_id}")
    ...
    if agent_id:
        return {
            "success": True,
            "message": f"Successfully installed Obi Chat agent for user {account_id}",
            "agent_id": agent_id
        }
    else:
        raise HTTPException(
            status_code=500, 
            detail=f"Failed to install Obi Chat agent for user {account_id}"
        )
