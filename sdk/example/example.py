...
async def main():
    ...
    # Create the MCP tools client with the URL of the MCP server that's accessible by the Obi Chat instance
    mcp_tools = kortix.MCPTools(
        "http://localhost:4000/mcp/",  # Since we are running Obi Chat locally, we can use the local URL
        "Kortix",
