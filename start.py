...
def print_manual_instructions():
    """Prints instructions for manually starting Obi Chat services."""
    print(f"\n{Colors.BLUE}{Colors.BOLD}🚀 Manual Startup Instructions{Colors.ENDC}\n")

    print("To start Obi Chat, you need to run these commands in separate terminals:\n")
    ...
    print("Once all services are running, access Obi Chat at: http://localhost:3000\n")
...
def main():
    ...
    if "--help" in sys.argv:
        ...
        print("Manage Obi Chat services based on your setup method")
        print("\nOptions:")
    ...
    if setup_method == "manual":
        ...
        print("Managing all Obi Chat services with Docker Compose...\n")
        ...
        if is_up:
            action = "stop"
            msg = "🛑 Stop all Obi Chat services? [y/N] "
        else:
            action = "start"
            msg = "⚡ Start all Obi Chat services? [Y/n] "
        ...
        if action == "stop":
            subprocess.run(["docker", "compose", "down"], shell=IS_WINDOWS)
            print(f"\n{Colors.GREEN}✅ All Obi Chat services stopped.{Colors.ENDC}")
        else:
            subprocess.run(["docker", "compose", "up", "-d"], shell=IS_WINDOWS)
            print(f"\n{Colors.GREEN}✅ All Obi Chat services started.{Colors.ENDC}")
            print(f"{Colors.CYAN}🌐 Access Obi Chat at: http://localhost:3000{Colors.ENDC}")
