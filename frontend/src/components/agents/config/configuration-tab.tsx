...
export function ConfigurationTab({
  ...
  const handleSystemPromptChange = (value: string) => {
    ...
    if (!isSystemPromptEditable && isSunaAgent) {
      console.log('❌ System prompt edit blocked for Obi Chat agent');
      toast.error("System prompt cannot be edited", {
        description: "Obi Chat's system prompt is managed centrally and cannot be changed.",
      });
      return;
    }
  ...
  const handleToolsChange = (tools: Record<string, boolean | { enabled: boolean; description: string }>) => {
    ...
    if (!areToolsEditable && isSunaAgent) {
      console.log('❌ Tools edit blocked for Obi Chat agent');
      toast.error("Tools cannot be modified", {
        description: "Obi Chat's default tools are managed centrally and cannot be changed.",
      });
      return;
    }
        ...
        {isSunaAgent && (
          <div className="mb-4 p-4 bg-primary/10 border border-primary-200 rounded-xl">
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="font-semibold text-primary-800">Obi Chat Default Agent</span>
            </div>
            <p className="text-sm text-primary-700">
              This is Obi Chat's default agent with centrally managed system prompt and tools. 
